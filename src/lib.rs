...
macro_rules! def_oid {
    ($name: ident, $oid: expr) => {
        pub static $name: Lazy<ObjectIdentifier> = Lazy::new(|| ObjectIdentifier::from_slice(&$oid));
    };
}

def_oid!(OID_DATA_CONTENT_TYPE, [1, 2, 840, 113_549, 1, 7, 1]);
def_oid!(OID_ENCRYPTED_DATA_CONTENT_TYPE, [1, 2, 840, 113_549, 1, 7, 6]);
def_oid!(OID_FRIENDLY_NAME, [1, 2, 840, 113_549, 1, 9, 20]);
def_oid!(OID_LOCAL_KEY_ID, [1, 2, 840, 113_549, 1, 9, 21]);
def_oid!(OID_CERT_TYPE_X509_CERTIFICATE, [1, 2, 840, 113_549, 1, 9, 22, 1]);
def_oid!(OID_CERT_TYPE_SDSI_CERTIFICATE, [1, 2, 840, 113_549, 1, 9, 22, 2]);
def_oid!(
    OID_PBE_WITH_SHA_AND3_KEY_TRIPLE_DESCBC,
...
def_oid!(OID_CERT_BAG, [1, 2, 840, 113_549, 1, 12, 10, 1, 3]);
def_oid!(OID_CRL_BAG, [1, 2, 840, 113_549, 1, 12, 10, 1, 4]);
def_oid!(OID_SECRET_BAG, [1, 2, 840, 113_549, 1, 12, 10, 1, 5]);
def_oid!(OID_SAFE_CONTENTS_BAG, [1, 2, 840, 113_549, 1, 12, 10, 1, 6]);
def_oid!(OID_PKCS5_PBES2, [1, 2, 840, 113_549, 1, 5, 13]);
def_oid!(OID_PKCS5_PBKDF2, [1, 2, 840, 113_549, 1, 5, 12]);
def_oid!(OID_AES256_CBC, [2, 16, 840, 1, 101, 3, 4, 1, 42]);
def_oid!(OID_AES192_CBC, [2, 16, 840, 1, 101, 3, 4, 1, 22]);
def_oid!(OID_AES128_CBC, [2, 16, 840, 1, 101, 3, 4, 1, 2]);
def_oid!(OID_HMAC_WITH_SHA256, [1, 2, 840, 113_549, 2, 9]);
def_oid!(OID_HMAC_WITH_SHA1, [1, 2, 840, 113_549, 2, 7]);

#[derive(Debug)]
pub struct Pkcs12Password<'a> {
    password: &'a str,
    bmp_string: Vec<u8>,
}

impl<'a> Pkcs12Password<'a> {
    pub fn new(password: &'a str) -> Self {
        let utf16: Vec<u16> = password.encode_utf16().collect();

        let mut bmp_string = Vec::with_capacity(utf16.len() * 2 + 2);
        for c in utf16 {
            bmp_string.push((c / 256) as u8);
            bmp_string.push((c % 256) as u8);
        }
        bmp_string.push(0x00);
        bmp_string.push(0x00);
        Self { password, bmp_string }
    }
...
impl EncryptedContentInfo {
    pub fn parse(r: BERReader) -> Result<Self, ASN1Error> {
        r.read_sequence(|r| {
            ...
            let encrypted_content = r.next().read_tagged_implicit(Tag::context(0), |r| r.read_bytes())?;
    ...
    pub fn data(&self, password: &Pkcs12Password) -> Option<Vec<u8>> {
    ...
    pub fn from_safe_bags(safe_bags: &[SafeBag], password: &Pkcs12Password) -> Option<EncryptedContentInfo> {
        ...
        let encrypted_content = pbe_with_sha1_and40_bit_rc2_cbc_encrypt(&data, password, &salt, ITERATIONS)?;
        let content_encryption_algorithm = AlgorithmIdentifier::PbewithSHAAnd40BitRC2CBC(Pkcs12PbeParams {
            salt,
...
impl EncryptedData {
    pub fn parse(r: BERReader) -> Result<Self, ASN1Error> {
        r.read_sequence(|r| {
            ...
            Ok(EncryptedData { encrypted_content_info })
        })
    }
    pub fn data(&self, password: &Pkcs12Password) -> Option<Vec<u8>> {
    ...
    pub fn from_safe_bags(safe_bags: &[SafeBag], password: &Pkcs12Password) -> Option<Self> {
...
impl ContentInfo {
    pub fn parse(r: BERReader) -> Result<Self, ASN1Error> {
        r.read_sequence(|r| {
            ...
            Ok(ContentInfo::OtherContext(OtherContext { content_type, content }))
        })
    }
    pub fn data(&self, password: &Pkcs12Password) -> Option<Vec<u8>> {
        match self {
    ...
    pub fn write(&self, w: DERWriter) {
        match self {
            ...
            ContentInfo::OtherContext(other) => w.write_sequence(|w| {
                ...
                w.next().write_tagged(Tag::context(0), |w| w.write_der(&other.content))
            }),
        }
    }
    pub fn to_der(&self) -> Vec<u8> {
...
impl Pkcs5Pbes2Params {
    pub fn parse(r: BERReader) -> Result<Self, ASN1Error> {
        r.read_sequence(|r| {
            ...
            let seq_cipher = r.next();
            let (cipher, iv) = seq_cipher.read_sequence(|r| {
                let cipher_oid = r.next().read_oid()?;
                let cipher_iv = r.next().read_bytes()?;
                Ok((cipher_oid, cipher_iv))
            })?;
    ...
    pub fn write(&self, w: DERWriter) {
        w.write_sequence(|w| {
            w.next().write_sequence(|w| {
                w.next().write_oid(&OID_PKCS5_PBKDF2);
                w.next().write_sequence(|w| {
                    w.next().write_bytes(&self.salt);
                    w.next().write_u64(self.iterations);
                    w.next().write_sequence(|w| {
                        w.next().write_oid(&self.hasher);
                        w.next().write_null();
                    });
                });
                w.next().write_sequence(|w| {
                    w.next().write_oid(&self.cipher);
...
impl AlgorithmIdentifier {
    pub fn parse(r: BERReader) -> Result<Self, ASN1Error> {
        r.read_sequence(|r| {
            ...
            if algorithm_type == *OID_PKCS5_PBES2 {
                let params = Pkcs5Pbes2Params::parse(r.next())?;
                return Ok(AlgorithmIdentifier::Pkcs5PBES2(params));
            }
    ...
    pub fn decrypt_pbe(&self, ciphertext: &[u8], password: &Pkcs12Password) -> Option<Vec<u8>> {
        match self {
            ...
            AlgorithmIdentifier::PbeWithSHAAnd3KeyTripleDESCBC(param) => {
                pbe_with_sha_and3_key_triple_des_cbc(ciphertext, password, &param.salt, param.iterations)
            }
            AlgorithmIdentifier::Pkcs5PBES2(param) => pbe_pkcs5_scheme_2(ciphertext, password, param),
    ...
    pub fn write(&self, w: DERWriter) {
        w.write_sequence(|w| match self {
            ...
            AlgorithmIdentifier::Pkcs5PBES2(p) => {
                w.next().write_oid(&OID_PKCS5_PBES2);
                p.write(w.next());
            }
...
impl MacData {
    ...
    pub fn verify_mac(&self, data: &[u8], password: &Pkcs12Password) -> bool {
        debug_assert_eq!(self.mac.digest_algorithm, AlgorithmIdentifier::Sha1);
        let key = pbepkcs12sha1(&password.bmp_string, &self.salt, self.iterations as u64, 3, 20);
    ...
    pub fn new(data: &[u8], password: &Pkcs12Password) -> MacData {
        let salt = rand().unwrap();
        let key = pbepkcs12sha1(&password.bmp_string, &salt, ITERATIONS, 3, 20);
...
impl PFX {
    ...
    pub fn new_with_cas(
        ...
        let salt = rand()?.to_vec();
        let encrypted_data = pbe_with_sha_and3_key_triple_des_cbc_encrypt(key_der, password, &salt, ITERATIONS)?;
    ...
    pub fn bags(&self, password: &Pkcs12Password) -> Result<Vec<SafeBag>, ASN1Error> {
        let data = self
            .auth_safe
            .data(password)
            .ok_or_else(|| ASN1Error::new(ASN1ErrorKind::Invalid))?;
    ...
    pub fn cert_bags(&self, password: &Pkcs12Password) -> Result<Vec<Vec<u8>>, ASN1Error> {
        self.cert_x509_bags(password)
    }
    //DER-encoded X.509 certificate
    pub fn cert_x509_bags(&self, password: &Pkcs12Password) -> Result<Vec<Vec<u8>>, ASN1Error> {
        let mut result = vec![];
    ...
    pub fn cert_sdsi_bags(&self, password: &Pkcs12Password) -> Result<Vec<String>, ASN1Error> {
        let mut result = vec![];
        for safe_bag in self.bags(password)? {
    ...
    pub fn key_bags(&self, password: &Pkcs12Password) -> Result<Vec<Vec<u8>>, ASN1Error> {
        let mut result = vec![];
        for safe_bag in self.bags(password)? {
    ...
    pub fn verify_mac(&self, password: &Pkcs12Password) -> bool {
        if let Some(mac_data) = &self.mac_data {
...
fn pbe_with_sha1_and40_bit_rc2_cbc(
    ...
    let dk = pbepkcs12sha1(&password.bmp_string, salt, iterations, 1, 5);
    let iv = pbepkcs12sha1(&password.bmp_string, salt, iterations, 2, 8);
...
fn pbe_with_sha_and3_key_triple_des_cbc(
    ...
    let dk = pbepkcs12sha1(&password.bmp_string, salt, iterations, 1, 24);
    let iv = pbepkcs12sha1(&password.bmp_string, salt, iterations, 2, 8);
...
fn pbe_pkcs5_scheme_2(data: &[u8], password: &Pkcs12Password, param: &Pkcs5Pbes2Params) -> Option<Vec<u8>> {
    ...
    let res = if param.cipher.eq(&OID_AES256_CBC) {
        ...
        d.decrypt_padded_vec_mut::<Pkcs7>(data).ok()?
    } else if param.cipher.eq(&OID_AES192_CBC) {
...
fn _pbe_pkcs5_scheme_2_encrypt(data: &[u8], password: &str, param: &Pkcs5Pbes2Params) -> Option<Vec<u8>> {
    ...
    // encrypt
    let res = if param.cipher.eq(&OID_AES256_CBC) {
        let d = Aes256CbcEnc::new(key.as_bytes().into(), param.iv.as_slice().into());
        d.encrypt_padded_vec_mut::<Pkcs7>(data)
    } else if param.hasher.eq(&OID_AES192_CBC) {
        let d = Aes192CbcEnc::new(key.as_bytes().into(), param.iv.as_slice().into());
        d.encrypt_padded_vec_mut::<Pkcs7>(data)
    } else if param.hasher.eq(&OID_AES128_CBC) {
        let d = Aes128CbcEnc::new(key.as_bytes().into(), param.iv.as_slice().into());
        d.encrypt_padded_vec_mut::<Pkcs7>(data)
    } else {
        return None;
    };
    ...
    pub fn parse(r: BERReader) -> Result<Self, ASN1Error> {
        r.read_sequence(|r| {
            ...
            if oid == *OID_CERT_TYPE_SDSI_CERTIFICATE {
                let sdsi = r.next().read_tagged(Tag::context(0), |r| r.read_ia5_string())?;
    ...
    pub fn write(&self, w: DERWriter) {
        w.write_sequence(|w| match self {
            ...
            CertBag::SDSI(sdsi) => {
                ...
                w.next().write_tagged(Tag::context(0), |w| w.write_ia5_string(sdsi));
            }
        })
    }
}
...
impl EncryptedPrivateKeyInfo {
    ...
    pub fn decrypt(&self, password: &Pkcs12Password) -> Option<Vec<u8>> {
        self.encryption_algorithm.decrypt_pbe(&self.encrypted_data, password)
    }
}
...
impl SafeBagKind {
    pub fn parse(r: BERReader, bag_id: ObjectIdentifier) -> Result<Self, ASN1Error> {
        ...
        if bag_id == *OID_PKCS8_SHROUDED_KEY_BAG {
            return Ok(SafeBagKind::Pkcs8ShroudedKeyBag(EncryptedPrivateKeyInfo::parse(r)?));
        }
    ...
    pub fn get_key(&self, password: &Pkcs12Password) -> Option<Vec<u8>> {
...
impl SafeBag {
    pub fn parse(r: BERReader) -> Result<Self, ASN1Error> {
        r.read_sequence(|r| {
            ...
            let bag = r.next().read_tagged(Tag::context(0), |r| SafeBagKind::parse(r, oid))?;
...
fn test_create_p12() {
    ...
    let password = &Pkcs12Password::new("changeit");
    let p12 = PFX::new(&cert, &key, Some(&ca), password, "look").unwrap().to_der();
...
fn test_create_p12_without_password() {
    ...
    let p12 = PFX::new(&cert, &[], Some(&ca), password, "look").unwrap().to_der();
...
#[test]
fn test_bmp_string() {
    let value = Pkcs12Password::new("Beavis");
    assert_eq!(
        value.bmp_string,
...
fn test_pbepkcs12sha1() {
    use hex_literal::hex;
    let pass = Pkcs12Password::new("");
    assert_eq!(&pass.bmp_string, &vec![0, 0]);
    let salt = hex!("9af4702958a8e95c");
    let iterations = 2048;
    let id = 1;
    let size = 24;
    let result = pbepkcs12sha1(&pass.bmp_string, &salt, iterations, id, size);
